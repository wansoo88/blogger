...
### ADR-009: GA4 Data API 연동
- **날짜**: 2026-03-14
- **상태**: 채택
- **결정**: Google Analytics 4 Data API v1beta를 통한 트래픽 모니터링 추가
- **이유**: AdSense 승인 심사 시 트래픽 데이터 확인 필수, Search Console만으로는 사용자 행동 데이터 부족
- **대안**: GA4 임베디드 대시보드 (복잡), Search Console만 사용 (트래픽 데이터 부재)
- **결과**: sessions/users/pageViews/bounceRate/소스별 트래픽 추적 가능, 미설정 시 graceful fallback

### ADR-010: 품질 기준 85점 상향 및 검사 항목 확대
- **날짜**: 2026-03-14
- **상태**: 채택
- **결정**: 자동 승인 기준을 70점에서 85점으로 상향, 키워드 밀도/가독성/AI 도입부 검사 추가
- **이유**: 70점 기준은 AdSense 승인에 불충분, AI 생성 콘텐츠의 자연스러움 검증 필요
- **대안**: 전수 수동 검수 (시간 소모), 기존 70점 유지 (품질 위험)
- **결과**: 9개 항목 100점 만점, 85점 이상 자동 승인, 미통과 시 개선 제안 자동 제공

### ADR-011: Alert 평가 시스템 구현
- **날짜**: 2026-03-14
- **상태**: 채택
- **결정**: 스키마에만 존재하던 6개 AlertType에 대한 실제 평가 로직 구현
- **이유**: 색인 지연, 노출 0, 발행 실패 등 운영 이슈 자동 감지 필요
- **대안**: 수동 대시보드 확인 (누락 위험), 외부 모니터링 도구 (비용)
- **결과**: 자동 감지/해소, PUBLISH_FAILURE와 INDEXING_DELAY는 이메일 알림 연동
